...
function months(weeks: ContributionYear["weeks"]) {
  return weeks
    .map((week, index) => {
      ...
      return day
        ? { label: month.format(new Date(`${day.date}T12:00:00`)), index }
        : null;
    })
    .filter((value): value is { label: string; index: number } =>
      Boolean(value),
    );
}
...
export function ContributionGraph({ years }: { years: ContributionYear[] }) {
  ...
  const labels = useMemo(
    () => (selected ? months(selected.weeks) : []),
    [selected],
  );
  ...
  return (
    <section className="contribution-shell" aria-labelledby="github-title">
      <header className="contribution-head">
        ...
        >
          <strong>{selected.total.toLocaleString()}</strong>
          <span>contributions / {selected.year}</span>
        </section>
      </header>
      {years.length > 1 && (
        <nav className="year-switcher" aria-label="Contribution year">
          {years.map((item) => (
            <button
              key={item.year}
              type="button"
              aria-pressed={item.year === selected.year}
      ...
      <section
        ...
        tabIndex={0}
        aria-label={`GitHub contribution activity for ${selected.year}`}
      >
        <section className="month-row" aria-hidden="true">
          {labels.map((item) => (
            <span
        ...
        <section className="calendar">
          <section className="weekday-labels" aria-hidden="true">
            <span>Mon</span>
            <span>Wed</span>
            <span>Fri</span>
          </section>
          <section className="calendar-weeks">
            {selected.weeks.map((week, weekIndex) => (
              <section
                className="calendar-week"
                key={`${selected.year}-${weekIndex}`}
              >
                {week.map((day, dayIndex) => (
                  <motion.button
                    ...
                    transition={{
                      duration: 0.18,
                      delay: Math.min((weekIndex * 7 + dayIndex) * 0.003, 0.22),
                    }}
                  />
                ))}
              </section>
